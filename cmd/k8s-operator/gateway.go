...
func (a *GatewayReconciler) maybeProvision(ctx context.Context, logger *zap.SugaredLogger, gate *gatewayv1.Gateway) error {
	...
	at := gatewayv1.HostnameAddressType
...
func (a *GatewayReconciler) addWebListener(gate *gatewayv1.Gateway, lis *gatewayv1.Listener, sc *ipn.ServeConfig) {
	hostPort := ipn.HostPort(fmt.Sprintf("${TS_CERT_DOMAIN}:%d", lis.Port))
...
func (a *GatewayReconciler) configureHTTPRoutes(ctx context.Context, gate *gatewayv1.Gateway, sc *ipn.ServeConfig) error {
	hrs, err := a.gatewayClient.
		GatewayV1().
		HTTPRoutes(gate.Namespace).
			...
			for _, rule := range hr.Spec.Rules {
				for _, backendRef := range rule.BackendRefs {
					if backendRef.Group != nil && *backendRef.Group != gatewayv1.Group("") {
						a.recorder.Eventf(&hr, corev1.EventTypeWarning, "InvalidGatewayBackend", "backend %s group is not supported", *backendRef.Group)
						continue
					}
					if backendRef.Kind != nil && *backendRef.Kind != gatewayv1.Kind("Service") {
						a.recorder.Eventf(&hr, corev1.EventTypeWarning, "InvalidGatewayBackend", "backend %s kind is not supported", *backendRef.Kind)
						continue
					}
					key := types.NamespacedName{Name: string(backendRef.Name)}
					if backendRef.Namespace != nil {
						key.Namespace = string(*backendRef.Namespace)
					} else {
						key.Namespace = gate.Namespace
					}
					var svc corev1.Service
					if err := a.Get(ctx, key, &svc); err != nil {
						a.recorder.Eventf(&hr, corev1.EventTypeWarning, "InvalidGatewayBackend", "failed to get service %q: %v", backendRef.Name, err)
						continue
					}
					if svc.Spec.ClusterIP == "" || svc.Spec.ClusterIP == "None" {
						a.recorder.Eventf(&hr, corev1.EventTypeWarning, "InvalidGatewayBackend", "backend %s has invalid ClusterIP", backendRef.Name)
						continue
					}
					if backendRef.Port == nil || *backendRef.Port == 0 {
						a.recorder.Eventf(&hr, corev1.EventTypeWarning, "InvalidGatewayBackend", "backend %q has invalid port", backendRef.Name)
						continue
					}
					for _, match := range rule.Matches {
						if match.Path != nil {
							if match.Path.Type != nil && *match.Path.Type != gatewayv1.PathMatchPathPrefix {
								a.recorder.Eventf(&hr, corev1.EventTypeWarning, "InvalidGatewayBackend", "backend %q path match type %s is not supported", backendRef.Name, match.Path.Type)
								continue
							}
							if match.Path.Value == nil {
								a.recorder.Eventf(&hr, corev1.EventTypeWarning, "InvalidGatewayBackend", "backend %q path match value is nil", backendRef.Name)
								continue
							}
							proto := "http://"
							if (backendRef.Port != nil && *backendRef.Port == 443) || backendRef.Name == "https" {
								proto = "https+insecure://"
							}
							sc.Web[ipn.HostPort(fmt.Sprintf("${TS_CERT_DOMAIN}:%d", *parentRef.Port))].Handlers[*match.Path.Value] = &ipn.HTTPHandler{
								Proxy: proto + svc.Spec.ClusterIP + ":" + fmt.Sprint(*backendRef.Port) + *match.Path.Value,
							}
						}
					}
				}
...
func (a *GatewayReconciler) configureTCPRoutes(ctx context.Context, gate *gatewayv1.Gateway, sc *ipn.ServeConfig) error {
	tr, err := a.gatewayClient.
		GatewayV1alpha2().
		TCPRoutes(gate.Namespace).
		List(ctx, metav1.ListOptions{})
	if err != nil {
		return fmt.Errorf("failed to list TCPRoutes: %w", err)
	}

	for _, tcpRoute := range tr.Items {
		if tcpRoute.Spec.ParentRefs == nil || len(tcpRoute.Spec.ParentRefs) == 0 {
			a.recorder.Eventf(&tcpRoute, corev1.EventTypeWarning, "InvalidGatewayParentRef", "no parent refs specified")
			continue
		}
		for _, parentRef := range tcpRoute.Spec.ParentRefs {
			if a.isGatewayMatchingParentRef(gate, parentRef) {
				a.recorder.Eventf(&tcpRoute, corev1.EventTypeWarning, "InvalidGatewayParentRef", "parent ref %s does not match Gateway", parentRef.Name)
				continue
			}
			for _, rule := range tcpRoute.Spec.Rules {
				for _, backendRef := range rule.BackendRefs {
					if backendRef.Group != nil && *backendRef.Group != gatewayv1alpha2.Group("") {
						a.recorder.Eventf(&tcpRoute, corev1.EventTypeWarning, "InvalidGatewayBackend", "backend %s group is not supported", *backendRef.Group)
						continue
					}
					if backendRef.Kind != nil && *backendRef.Kind != gatewayv1alpha2.Kind("Service") {
						a.recorder.Eventf(&tcpRoute, corev1.EventTypeWarning, "InvalidGatewayBackend", "backend %s kind is not supported", *backendRef.Kind)
						continue
					}
					key := types.NamespacedName{Name: string(backendRef.Name)}
					if backendRef.Namespace != nil {
					...
					var svc corev1.Service
					if err := a.Get(ctx, key, &svc); err != nil {
						a.recorder.Eventf(&tcpRoute, corev1.EventTypeWarning, "InvalidGatewayBackend", "failed to get service %q: %v", backendRef.Name, err)
						continue
					}
					if svc.Spec.ClusterIP == "" || svc.Spec.ClusterIP == "None" {
						a.recorder.Eventf(&tcpRoute, corev1.EventTypeWarning, "InvalidGatewayBackend", "backend %s has invalid ClusterIP", backendRef.Name)
						continue
					}
					if backendRef.Port == nil || *backendRef.Port == 0 {
						a.recorder.Eventf(&tcpRoute, corev1.EventTypeWarning, "InvalidGatewayBackend", "backend %q has invalid port", backendRef.Name)
						continue
					}
					if parentRef.Port != nil {
						sc.TCP[uint16(*parentRef.Port)].TCPForward = fmt.Sprintf("%s:%d", svc.Spec.ClusterIP, *backendRef.Port)
					}
				}
			}
		}
