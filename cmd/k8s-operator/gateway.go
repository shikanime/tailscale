		...
		for _, parentRef := range hr.Spec.ParentRefs {
			...
			for _, rule := range hr.Spec.Rules {
				for _, br := range rule.BackendRefs {
					...
					key := types.NamespacedName{Name: string(br.Name)}
					if br.Namespace != nil {
						key.Namespace = string(*br.Namespace)
					} else {
					...
					if br.Port == nil || *br.Port == 0 {
						a.recorder.Eventf(&hr, corev1.EventTypeWarning, "InvalidGatewayBackend", "backend %q has invalid port", br.Name)
						continue
					}
					for _, match := range rule.Matches {
						if match.Path != nil {
							...
							proto := "http://"
							if (br.Port != nil && *br.Port == 443) || br.Name == "https" {
...
func (a *GatewayReconciler) configureTCPRoutes(ctx context.Context, gate *gatewayv1.Gateway, sc *ipn.ServeConfig) error {
	...
	for _, tcpRoute := range tr.Items {
		...
		for _, pr := range tcpRoute.Spec.ParentRefs {
			if a.isGatewayMatchingParentRef(gate, pr) {
				a.recorder.Eventf(&tcpRoute, corev1.EventTypeWarning, "InvalidGatewayParentRef", "parent ref %s does not match Gateway", pr.Name)
				continue
			}
			for _, rule := range tcpRoute.Spec.Rules {
				for _, br := range rule.BackendRefs {
					if err := a.validateBackendRef(br); err != nil {
						a.recorder.Eventf(&tcpRoute, corev1.EventTypeWarning, "InvalidGatewayBackend", "invalid backend ref %v: %v", br, err)
						continue
					}
					key := types.NamespacedName{Name: string(br.Name)}
					if br.Namespace != nil {
					...
					var svc corev1.Service
					if err := a.Get(ctx, key, &svc); err != nil {
						a.recorder.Eventf(&tcpRoute, corev1.EventTypeWarning, "InvalidGatewayBackend", "failed to get service %q: %v", br.Name, err)
						continue
					}
					if svc.Spec.ClusterIP == "" || svc.Spec.ClusterIP == "None" {
						a.recorder.Eventf(&tcpRoute, corev1.EventTypeWarning, "InvalidGatewayBackend", "backend %s has invalid ClusterIP", br.Name)
						continue
					}
					if br.Port == nil || *br.Port == 0 {
						a.recorder.Eventf(&tcpRoute, corev1.EventTypeWarning, "InvalidGatewayBackend", "backend %q has invalid port", br.Name)
						continue
					}
					if pr.Port != nil {
