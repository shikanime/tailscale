...
func runReconcilers(opts reconcilerOpts) {
	...
	gatewayClient, err := gatewayclient.NewForConfig(mgr.GetConfig())
	if err != nil {
		startlog.Fatalf("could not create gateway sigs client: %v", err)
	}
...
// serviceHandlerForGateway returns a handler for Service events for gateway
// reconciler that ensures that if the Service associated with an event is of
// interest to the reconciler, the associated Gateway(s) gets reconciled.
// The Services of interest are backend Services for Gateway routes (HTTPRoute
// and TCPRoute) and managed Services for a StatefulSet for a proxy configured
// for Gateway.
