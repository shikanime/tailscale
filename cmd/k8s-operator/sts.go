...
func applyProxyClassToStatefulSet(pc *tsapi.ProxyClass, ss *appsv1.StatefulSet, stsCfg *tailscaleSTSConfig, logger *zap.SugaredLogger) *appsv1.StatefulSet {
	...
	if pc.Spec.StatefulSet == nil || pc.Spec.StatefulSet.Pod == nil {
		maybeDefaultAffinityFromServiceSelector(ss, stsCfg, pc)
		return ss
	}
...
func maybeDefaultAffinityFromServiceSelector(ss *appsv1.StatefulSet, stsCfg *tailscaleSTSConfig, pc *tsapi.ProxyClass) {
	if ss == nil ||
		ss.Spec.Template.Spec.Affinity != nil ||
		stsCfg == nil ||
	...
	if stsCfg.TailnetTargetFQDN != "" || stsCfg.TailnetTargetIP != "" {
		return
	}
	if stsCfg.ClusterTargetIP == "" && stsCfg.ServeConfig == nil {
		return
	}
