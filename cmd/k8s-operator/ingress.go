...
func (a *IngressReconciler) maybeProvision(ctx context.Context, logger *zap.SugaredLogger, ing *networkingv1.Ingress) error {
	...
	backendSelector := a.serviceSelectorForIngress(ctx, ing, logger)
	sts := &tailscaleSTSConfig{
		...
		ClusterTargetServiceSelector: backendSelector,
...
func (a *IngressReconciler) serviceSelectorForIngress(ctx context.Context, ing *networkingv1.Ingress, logger *zap.SugaredLogger) map[string]string {
	if ing == nil {
		return nil
	}
	...
	for _, svcName := range serviceNames {
		...
		return maps.Clone(svc.Spec.Selector)
	}
	return nil
}
